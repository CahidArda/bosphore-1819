...
- **Counts:** `LABEL_COUNT` labels, of which `UNCERTAIN_COUNT` are flagged uncertain. Bounding boxes are stored as fractions of the full image (`[x, y, w, h]`), so they are independent of which scan resolution is displayed.
